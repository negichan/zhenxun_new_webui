...
export const useSystemStore = defineStore("system", () => {
    ...
    const messages = computed(() =>
        socketStore.getMessages("system_status", "latest"),
    );
     ...
     * 手动获取更新提交数据
     */
    const fetchCommits = async (isRetry = false) => {
        commitsLoading.value = true;

        try {
            const res = await dashboardApi.getCommits({
                timeout: 10000, // 10秒超时
            });

            if (res?.success) {
                Object.assign(commits, res.data || []);
            }
        } catch (error: any) {
            // axios 超时
        ...
        try {
            ...
            if (res?.success && res?.data) {
                Object.assign(count, {
                    ...
                    database_size:
                        res.data.database_size ?? count.database_size,
                });
                // 缓存到 localStorage
                localStorage.setItem("chat_call_count", JSON.stringify(count));
            }
