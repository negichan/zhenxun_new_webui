        ...
        <div v-if="!hideToolbar" class="editor-toolbar">
                    ...
                    <template #dropdown>
                        <el-dropdown-menu class="custom-dropdown-menu">
                            ...
                            >
                                <div class="dropdown-item-content">
                                    {{ theme.label }}
                                    <el-icon v-if="currentTheme === theme.value"
                                        ><Check
                                    /></el-icon>
                                </div>
                            </el-dropdown-item>
                    ...
                    <template #dropdown>
                        <el-dropdown-menu class="custom-dropdown-menu">
                            ...
                            >
                                <div class="dropdown-item-content">
                                    {{ eol.label }}
                                    <el-icon v-if="currentEOL === eol.value"
                                        ><Check
                                    /></el-icon>
                                </div>
                            </el-dropdown-item>
            ...
            <div class="toolbar-right">
                <span
                    class="dropdown-link"
                    @click="handleReset"
                    :class="{ 'cursor-not-allowed opacity-50': !isDirty }"
                >
...
const currentLanguage = ref(
    props.language === "auto"
        ? "自动检测"
        : languages.find((l) => l.value === props.language)?.label ||
              "Plain Text",
);
...
const eols = computed(() => {
    ...
    return [
        {
            label: "LF (Linux / Unix)",
            value: monaco.value.editor.EndOfLineSequence.LF,
        },
        {
            label: "CRLF (Windows)",
            value: monaco.value.editor.EndOfLineSequence.CRLF,
        },
    ];
});
...
const loadMonaco = async () => {
    const monacoModule = await import("monaco-editor-core");
...
const changeLanguage = (lang: string) => {
    ...
    if (lang === "auto") {
            ...
            if (ext) {
                ...
                if (detectedLang) {
                    ...
                    currentLanguage.value =
                        languages.find((l) => l.value === detectedLang)
                            ?.label || "Plain Text";
                    return;
                }
            }
        ...
        currentLanguage.value =
            languages.find((l) => l.value === lang)?.label || "Plain Text";
    }
};
